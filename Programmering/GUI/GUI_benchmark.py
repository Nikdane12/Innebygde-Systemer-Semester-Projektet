...
_pump_in1 = OutputDevice(21, initial_value=False)
...
def set_pump(value):
    ...
    pump_label.config(text=f"Power: {power}% | IN1: {'ON' if power > 0 else 'OFF'}")
    print(f"[PUMP] Power={power}%, IN1={'ON' if power > 0 else 'OFF'}")
